            ...
            <div v-motion="{
                ...
                <div class="flex gap-4 flex-wrap items-center justify-between mt-10">
                    ...
                    <UPopover class="relative z-[1050]" mode="hover" :ui="{ rounded: 'rounded-2xl' }">
                        <UButton size="xl" icon="i-heroicons-phone-16-solid">{{translations?.['main.call.number.button']}}</UButton>

                        <template #panel>
                            <div class="p-4 flex flex-col gap-4 bg-white shadow-lg rounded-2xl">
                                <a class="hover:text-cerulean-blue-500 transition-colors" :href="`tel:${translations?.['address.telefon1']}`">
                                    {{translations?.['address.telefon1']}}
                                </a>
                                <a class="hover:text-cerulean-blue-500 transition-colors" :href="`tel:${translations?.['address.telefon2']}`">
                                    {{translations?.['address.telefon2']}}
                                </a>
                                <a class="hover:text-cerulean-blue-500 transition-colors" :href="`tel:${translations?.['address.telefon3']}`">
